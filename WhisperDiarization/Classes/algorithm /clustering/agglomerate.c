                 ...
                 cluster_node_t **to_merge)
{
        ...
        int k = 0, idx;
        coord_t centroid = { 0 };
        centroid.dim = cluster->num_items;
        centroid.items = alloc_mem(centroid.dim, float);
        ...
        for (int i = 0; i < 2; ++i) {
                ...
                for (int index = 0; index < centroid.dim; ++index) {
                    centroid.items[index] += t->num_items * t->centroid.items[index];
                }
...
//                centroid.y += t->num_items * t->centroid.y;
        ...
        for (int i = 0; i < centroid.dim; i++) {
            float vvv = centroid.items[i];
            if (node->centroid.items == NULL) {
                node->centroid.items = alloc_mem(centroid.dim, float);
            }
            node->centroid.items[i] = vvv / k;
